...
def plot_precision_recall_vs_threshold(precisions, recalls, thresholds): 
    ...
    plt.savefig('./figures/modeling/{}_prec_rec_thresh.png'.format(plot_counter, bbox_inches='tight', transparent=True))
    plot_counter += 1
...
def full_report(clf, X_train, X_test, y_train, y_test, show_metrics=False, class_label=1): 
    ...
    plt.savefig('./figures/modeling/{}_precision_recall_curve.png'.format(plot_counter, bbox_inches='tight', transparent=True))
    plot_counter += 1
    plt.show()
    ...
    plt.savefig('./figures/modeling/{}_roc_auc_curve.png'.format(plot_counter, bbox_inches='tight', transparent=True))
    plot_counter += 1
    plt.show()
...
def examine_model(original_df, scores, scorer, target_name, numerical_feat, pos=0, class_label=1, reduce=False):  
    ...
    dump(clf, './models/{}_{}.joblib'.format(user, clf)) 
...
def plot_confusion_matrix(y_true, y_pred, show_metrics=True, beta=2):
    ...
    plt.savefig('./figures/modeling/{}_confusion_matrix.png'.format(plot_counter, bbox_inches='tight', transparent=True))
    plot_counter += 1
    plt.show()
...
def elbow_plot(cumsum, threshold): 
    ...
    plt.savefig('./figures/modeling/{}_elbow_plot.png'.format(plot_counter, bbox_inches='tight', transparent=True))
    plot_counter += 1
    plt.show()
...
def plot_threshold(precisions, recalls, thresholds, clf, recall_thresh=0.5):
    ...
    plt.savefig('./figures/modeling/{}_threshold.png'.format(plot_counter, bbox_inches='tight', transparent=True))
    plot_counter += 1
    plt.show()
    return thresh
...
def plot_univariate_dist(df, feature, color, plot_mean = True, plot_std = False, std_lab=False, kde = True, stat='count', truncate=True, auto_bins=False, spearmanr=False):
    ...
    if spearmanr == True:
        ...
        plt.text(std*0.5 + mean, max_height + max_height*.1, 'Spearman_r: {:.3f} \n p: {:.3f}'.format(r,p), fontsize=12)
        plt.savefig('./figures/eda/{}_dist.png'.format(feature), bbox_inches='tight', transparent=True)
...
def plot_distributions(df, feature, mapper, target=True, truncate=True):
    if target:
        ...
        plt.savefig('./figures/eda/{}_distribution.png'.format(feature), bbox_inches='tight', transparent=True)
        plt.show()
        ...
        while i < len(mapper):
            ...
            plt.savefig('./figures/eda/{}_{}_{}_distribution.png'.format(mapper[i][0], mapper[j][0], feature), bbox_inches='tight', transparent=True)
            plt.show()
        ...
        plt.xlabel('')
        plt.ylabel('')
        plt.savefig('./figures/eda/{}_boxplot.png'.format(feature), bbox_inches='tight', transparent=True)
        ...
        plt.savefig('./figures/eda/{}_kde.png'.format(feature), bbox_inches='tight', transparent=True)
        plt.show()
